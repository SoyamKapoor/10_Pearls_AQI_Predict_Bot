...
def get_secret(name, default=None):
    # Prefer Streamlit secrets, fallback to .env (os.environ)
...
def fetch_features_from_hopsworks():
    try:
        api_key = get_secret("HOPSWORKS_API_KEY")
...
# # def get_secret(name, default=None):
# #     # Prefer Streamlit Cloud secrets, fall back to OS env (for local .env use)
# #     return st.secrets[name] if name in st.secrets else os.getenv(name, default)
