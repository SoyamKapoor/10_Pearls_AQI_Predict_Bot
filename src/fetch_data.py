...
def fetch_current_aqi():
    ...
    try:
        ...
        # Local time for fetch
        utc_now = datetime.now(pytz.utc)
        pk_tz = pytz.timezone('Asia/Karachi')
        local_now = utc_now.astimezone(pk_tz)
        # Parse OpenWeather timestamp, fallback to fetch time if error
        try:
            ...
            dt_readable = local_now.strftime('%Y-%m-%d %H:%M:%S')
        record = {
            ...
            'coord_lat': coord_lat,
            'coord_lon': coord_lon,
            'main_aqi': measurement['main'].get('aqi', ''),
            'co': measurement['components'].get('co', ''),
            'no': measurement['components'].get('no', ''),
            'no2': measurement['components'].get('no2', ''),
            'o3': measurement['components'].get('o3', ''),
            'so2': measurement['components'].get('so2', ''),
            'pm2_5': measurement['components'].get('pm2_5', ''),
            'pm10': measurement['components'].get('pm10', ''),
            'nh3': measurement['components'].get('nh3', '')
        }
